        ...
        if(request.GET.get("agent",None) is not None):
            ...
            for cl in client:
                if cl.info_concession.agent_rattache.id == int(request.GET.get("agent")):
                    final_.append(cl)
            for a in ag:
                user_= User.objects.filter(pk=a.user.id)
                if user_.first().groups.filter(name="Agent secteur").exists():
                    pass
                else:
            ...
            for p in planneur:
                if p.groups.filter(name="Audit planneur").exists() and not p.groups.filter(name="Agent secteur").exists():
